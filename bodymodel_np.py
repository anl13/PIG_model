...
put them as the folder `./PIG_Model/core_txt_files/` (you can rename it as you like and change the 
`model_folder` variable below. ) 
'''
class BodyModelNumpy(object):
    def __init__(self, model_folder = "./core_txt_files/"):
    ...
    def readmodel(self, model_folder):
        ...
        return vertices_np, parents_np.astype(np.int64), joints_np, weights, faces_vert.astype(np.int64)
...
if __name__ == "__main__":
    bm = BodyModelNumpy()
    # This is the folder of BamaPig3D dataset. You can check it at https://github.com/anl13/MAMMAL_datasets
    BamaPig3D_path = "H:/examples/BamaPig3D/"
    for k in range(1): # can set up to 70
        frameid = k * 25 
        for pid in range(1): # can set up to 4
            ...
            # keypoints: [23, 3]
            keypoints = bm.regress_keypoints_pack() 
